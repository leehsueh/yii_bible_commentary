...
def read_kjv(filename):
    ...
    books[(book#, 'bookname')]['chp#']['verse#'] = (verse_id, verse text)
    """
    books = dict()
    curr_book = ''
    curr_chapter = 0
    ...
    for line in fi:
        if line.find(endtext) != -1:
            ...
            if verse_text != empty:
                ...
                id = id + 1
                books[curr_book][curr_chapter][curr_verse] = (id, ' '.join(verse_text))
                verse_text = empty
            break
...
def generate_sql(bible_dict, db_table, dbtype, filename):
    #insert into bibledb_verse (book, chapter, verse, verse_text) values ('Genesis', 1, 1, 'In the beginning, God created the heaven and the earth')
        ...
        for chapter in bible_dict[book]:
            for verse, text in bible_dict[book][chapter].items():
                if dbtype == 'mysql':
                    ...
                    + book[1] + "'," + str(chapter) + "," + str(verse) + ", \"" + text[1] + "\"\n);\n")
                else: #postgresql
                    text = text.replace("'","''")
                    sql_file.write("INSERT INTO " + db_table \
                    + " (id, book, chapter_ref, verse_ref, verse_text) values (nextval('" + db_table + "_id_seq'::regclass), '" \
                    + book[1] + "'," + str(chapter) + "," + str(verse) + ", '" + text[1] + "');\n")
...
generate_sql(bible, 'tbl_bible_verse', dbtype, filename + '_' + dbtype + '.sql')